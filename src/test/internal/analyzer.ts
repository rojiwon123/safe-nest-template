    ...
    export const report = (analyzed: IAnalyze, md: boolean): void => {
        ...
        if (analyzed.state === 0) {
            ...
            if (md) {
                ...
                logs.push('| :---: | :---: | :---: | :---: |');
                logs.push(
                    `| ${analyzed.total_count} | ${analyzed.total_count} | 0 | ${analyzed.total_time} ms |`,
                );
                logs.push('');
                logs.push('## Detail');
                logs.push('');
                analyzed.elapsed_time.forEach(([name, time]) =>
                    logs.push(`- ${name} : ${time} ms`),
                );
            } else {
            ...
            if (md) {
                ...
                logs.push(
                    `| ${analyzed.total_count} | ${analyzed.total_count - analyzed.failed_count} | ${analyzed.failed_count} |`,
                );
                logs.push('');
                logs.push('## Detail');
                logs.push('');
                analyzed.executions.forEach((exe) => {
                    ...
                    logs.push(
                        exe.error.stack ??
                            `${exe.error.name} ${exe.error.message}`,
                    );
                    logs.push('```');
                    logs.push('');
                });
                ...
                logs.push(
                    `${analyzed.failed_count} in ${analyzed.total_count} Tests Failed`,
                );
                logs.push('');
            }
        }
