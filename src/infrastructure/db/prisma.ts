...
export interface IPrismaClient extends Prisma.TransactionClient {
     ...
     */
    $safeTransaction: <T, E>(closure: (tx: Prisma.TransactionClient) => Promise<Result<T, E>>) => Promise<Result<T, E>>;
}
...
const once = Once.unit(() => {
    ...
    return client.$extends({
        client: {
            $safeTransaction: async <T, E>(closure: (tx: Prisma.TransactionClient) => Promise<Result<T, E>>): Promise<Result<T, E>> => {
                ...
                return client
                    ...
                    .catch((error: unknown) => {
                        if (error === rollback) return Result.Err<T, E>(rollback.cause as E);
